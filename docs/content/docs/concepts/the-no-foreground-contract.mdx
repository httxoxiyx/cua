...
On macOS, `cua-driver serve --async-click-feedback` keeps that supervision UI off the input critical path: click-family tools deliver the real action immediately while the synthetic cursor completes its glide and click pulse on the renderer thread. The pulse visualizes the attempted target; tool verification remains the source of truth for whether an action succeeded. The default remains synchronized for workflows that intentionally coordinate input timing with cursor arrival; explicit `move_cursor` and drag operations also retain their completion semantics.

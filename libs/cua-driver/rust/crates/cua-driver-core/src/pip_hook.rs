...
//! same frame without paying for a duplicate capture. A successful tree-only
//! observation instead authorizes a private, revalidated preview when needed;
//! that preview does not add image content or pointer authority to the result.
...
/// Bootstrap only an exact, successful tree-only observation. Unlike Ensure,
/// this carries observation provenance and may start private preview capture.
pub fn observe_pip_target(target: PipHookTarget) {
    if let Some(f) = PIP_EVENT_FN.get() {
        let _ = f(PipHookEvent::Observe(target));
    }
}

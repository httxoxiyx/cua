                ...
                pip_native_target(resolved_name, &args, &result)
            {
                ...
                match (update, png_bytes) {
                    ...
                    (PipUpdateKind::EnsureTarget, _) => {
                        if pip_tree_only_observation_is_resolved(resolved_name, &args, &result) {
                            pip_hook::observe_pip_target(target);
                        } else {
...
fn pip_native_target(
    ...
    if tool_name == "get_window_state" && result.is_error != Some(true) {
        if let Some(structured) = result.structured_content.as_ref() {
            if structured.get("screenshot_scope").and_then(Value::as_str)
                == Some("application_menu")
                || structured.get("screenshot_target").and_then(Value::as_str)
                    == Some("application_menu.visual_target")
            {
                // A menu image is not a document frame. Keep its physical
                // source explicit so PiP cannot overwrite it with the host's
                // stream. The same-PID logical marker is private; macOS must
                // independently revalidate the recorded menu proof.
                let pid = structured.get("pid")?.as_i64()?;
                let document = structured.get("window_id")?.as_u64()?;
                let menu = structured
                    .pointer("/application_menu/visual_target/window_id")?
                    .as_u64()?;
                let valid = pid > 0
                    && document > 0
                    && menu > 0
                    && menu != document
                    && structured.get("screenshot_scope").and_then(Value::as_str)
                        == Some("application_menu")
                    && structured.get("screenshot_target").and_then(Value::as_str)
                        == Some("application_menu.visual_target")
                    && structured
                        .get("screenshot_coordinates_actionable")
                        .and_then(Value::as_bool)
                        == Some(false)
                    && structured
                        .get("screenshot_frame_valid")
                        .and_then(Value::as_bool)
                        == Some(true)
                    && structured
                        .pointer("/application_menu/host_pid")
                        .and_then(Value::as_i64)
                        == Some(pid)
                    && structured
                        .pointer("/application_menu/host_window_id")
                        .and_then(Value::as_u64)
                        == Some(document)
                    && structured
                        .pointer("/application_menu/visual_target/pid")
                        .and_then(Value::as_i64)
                        == Some(pid)
                    && structured.get("transient_ui").is_none()
                    && structured.pointer("/selection/delegation").is_none();
                return valid.then_some((menu, pid, Some(pid), None));
            }
...
fn pip_tree_only_observation_is_resolved(
    tool_name: &str,
    args: &Value,
    result: &ToolResult,
) -> bool {
    if tool_name != "get_window_state"
        || result.is_error == Some(true)
        || args.get("include_screenshot").and_then(Value::as_bool) != Some(false)
    ...
    result.structured_content.as_ref().is_some_and(|state| {
        state
            .get("pid")
            .and_then(Value::as_i64)
            ...
            && state
                ...
                .and_then(Value::as_u64)
                .is_some_and(|id| id > 0)
            && state.get("elements_included").and_then(Value::as_bool) == Some(true)
            && state
                .get("snapshot_id")
                .and_then(Value::as_str)
                .is_some_and(|id| {
                    id.strip_prefix('s').is_some_and(|hex| {
...
mod capability_tests {
    ...
    #[test]
    fn pip_menu_image_requires_same_owner_visual_only_metadata() {
        let structured = serde_json::json!({
            "pid":42,"window_id":7,"screenshot_scope":"application_menu",
            "screenshot_coordinates_actionable":false,"screenshot_frame_valid":true,
            "screenshot_target":"application_menu.visual_target",
            "application_menu":{"host_pid":42,"host_window_id":7,"visual_target":{"pid":42,"window_id":9}},
        });
        ...
        assert_eq!(
            pip_native_target("get_window_state", &args, &result),
            Some((9, 42, Some(42), None))
        );
        for mutation in 0..6 {
            let mut bad = structured.clone();
            match mutation {
                0 => bad["application_menu"]["visual_target"]["pid"] = serde_json::json!(99),
                1 => bad["application_menu"]["host_window_id"] = serde_json::json!(8),
                2 => bad["screenshot_coordinates_actionable"] = serde_json::json!(true),
                3 => bad["screenshot_frame_valid"] = serde_json::json!(false),
                4 => bad["application_menu"]["visual_target"]["window_id"] = serde_json::json!(7),
                _ => bad["screenshot_scope"] = serde_json::json!("window"),
            }
            let result = ToolResult {
                structured_content: Some(bad),
                ..Default::default()
            };
            assert_eq!(
                pip_native_target("get_window_state", &args, &result),
    ...
    #[test]
    fn pip_tree_only_observation_requires_successful_resolved_output() {
        let args = serde_json::json!({"pid": 42, "include_screenshot": false});
        let mut result = ToolResult::text("tree").with_structured(serde_json::json!({
            "pid": 42, "window_id": 7, "snapshot_id": "s00000001",
            "elements_included": true
        }));
        assert!(pip_tree_only_observation_is_resolved(
            "get_window_state",
            &args,
            &result
        ));
    ...
    #[test]
    fn pip_tree_only_never_uses_action_args_or_a_missing_screenshot_as_proof() {
        let result = ToolResult::text("tree").with_structured(serde_json::json!({
            "pid": 42, "window_id": 7, "snapshot_id": "s00000001",
            "elements_included": true
        }));
        for args in [
            serde_json::json!({"pid": 42, "window_id": 7}),
            serde_json::json!({"pid": 42, "include_screenshot": true}),
            serde_json::json!({"pid": 42, "include_screenshot": false, "screenshot_out_file": "/tmp/frame.png"}),
        ] {
            assert!(!pip_tree_only_observation_is_resolved(
                "get_window_state",
                &args,
        ...
        let no_target = ToolResult::text("tree").with_structured(serde_json::json!({
            "snapshot_id": "s00000001", "elements_included": true
        }));
        assert!(!pip_tree_only_observation_is_resolved(
            "get_window_state",
            &serde_json::json!({"pid":42,"window_id":7,"include_screenshot":false}),
            &no_target
        ));
        for action in ["click", "set_value", "press_key"] {
            assert!(!pip_tree_only_observation_is_resolved(
                action,

...
pub enum ElementAncestry {
    ...
    /// requested one.
    ProvenDescendant,
    /// A live application-menu member whose owning application's focused
    /// window still matches the observed target. This is not window ancestry.
    ProvenApplicationMenu,
    /// A control in a distinct native popover with a fresh reciprocal AX
    /// attachment to the exact host. This is not physical window ancestry.
    ProvenAttachedPopover,
...
///   observation-only;
/// - an addressed element must prove ancestry to the requested window, or
///   prove live app-menu membership and exact app-local context exclusively
///   for the advertised application-menu semantic route;
...
pub fn decide_background_input(
    ...
    let addressed_target_proven = match (facts.element, action) {
        (ElementAncestry::ProvenApplicationMenu, BackgroundAction::ApplicationMenuSemantic) => true,
        (ElementAncestry::ProvenAttachedPopover, BackgroundAction::AttachedPopoverSemantic) => true,
        (ElementAncestry::ProvenApplicationMenu, _)
        | (_, BackgroundAction::ApplicationMenuSemantic)
        | (ElementAncestry::ProvenAttachedPopover, _)
        | (_, BackgroundAction::AttachedPopoverSemantic) => false,
        (ElementAncestry::NotAddressed | ElementAncestry::ProvenDescendant, _) => true,
        (ElementAncestry::OutsideTargetWindow | ElementAncestry::Unproven, _) => false,
    };
    if !addressed_target_proven {
        return refuse(
    ...
    match action {
        ...
        | BackgroundAction::AttachedPopoverSemantic => BackgroundInputDecision::Execute {
            verification: TargetBoundVerification {
        ...
        BackgroundAction::InsertText | BackgroundAction::GenericKey => {
            // This remains a refusal. Generic-key callers may explicitly
            // choose the existing exact-window foreground route; semantic
            // text callers retain their accessibility-first advice.
            let advice = if action == BackgroundAction::GenericKey {
                "foreground"
            } else {
                "accessibility"
            };
...
mod tests {
    ...
    const ALL_ACTIONS: [BackgroundAction; 6] = [
        BackgroundAction::AxSemantic,
        BackgroundAction::ApplicationMenuSemantic,
    ...
    #[test]
    fn application_menu_proof_only_authorizes_its_semantic_route() {
        let facts = BackgroundTargetFacts {
            element: ElementAncestry::ProvenApplicationMenu,
            // App-local focused-window proof, not global foreground or a
            // single-window application, owns the menu's document context.
            competing_keyboard_destinations: 1,
            ..matched_facts()
        };
        assert!(
            decide_background_input(TARGET, &facts, BackgroundAction::ApplicationMenuSemantic)
                .is_execute()
        );
        for action in ALL_ACTIONS {
            if action != BackgroundAction::ApplicationMenuSemantic {
                assert_eq!(
    ...
    #[test]
    fn application_menu_route_requires_menu_proof_not_generic_ancestry() {
        for element in [
    ...
    #[test]
    fn attached_popover_proof_only_authorizes_its_direct_semantic_route() {
        let facts = BackgroundTargetFacts {
            element: ElementAncestry::ProvenAttachedPopover,
            competing_keyboard_destinations: 1,
            ..matched_facts()
        };
        for action in ALL_ACTIONS {
            let decision = decide_background_input(TARGET, &facts, action);
            if action == BackgroundAction::AttachedPopoverSemantic {
        ...
        for element in [
            ElementAncestry::NotAddressed,
            ElementAncestry::ProvenDescendant,
            ElementAncestry::ProvenApplicationMenu,
            ElementAncestry::OutsideTargetWindow,
            ElementAncestry::Unproven,
        ] {
            assert_eq!(
    ...
    #[test]
    fn generic_key_refusal_advises_explicit_foreground_without_changing_text_advice() {
        for facts in [
            BackgroundTargetFacts {
                competing_keyboard_destinations: 1,
                ..matched_facts()
            ...
            for (action, advice) in [
                (BackgroundAction::GenericKey, "foreground"),
                (BackgroundAction::InsertText, "accessibility"),
            ] {
                let BackgroundInputDecision::Refuse(refusal) =
                    decide_background_input(TARGET, &facts, action)
                else {
                    panic!("background input must still refuse");
                };
                assert_eq!(refusal.advice, Some(advice));
            }
        }
    }
    ...
    fn ax_unresolved_target_is_observation_only() {
        ...
        for action in ALL_ACTIONS {
            let facts = if action == BackgroundAction::ApplicationMenuSemantic {
                BackgroundTargetFacts {
                    element: ElementAncestry::ProvenApplicationMenu,
                    ..facts.clone()
                }
            } else if action == BackgroundAction::AttachedPopoverSemantic {
                BackgroundTargetFacts {
                    element: ElementAncestry::ProvenAttachedPopover,
                    ..facts.clone()
                }

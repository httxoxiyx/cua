...
fn capture_plan_is_reusable(
    plan_identity: WindowCaptureIdentity,
    plan_includes_children: bool,
    live_identity: WindowCaptureIdentity,
    live_includes_children: bool,
) -> bool {
    plan_identity == live_identity && plan_includes_children == live_includes_children
}
...
fn build_window_capture_plan(
    ...
    // Desktop-independent window filter; composition is explicit below so
    // host child surfaces remain visible without misframing a focused sheet.
    // https://developer.apple.com/documentation/screencapturekit/sccontentfilter/init(desktopindependentwindow:)
...
fn screenshot_window_bytes_sck_inner(window_id: u32) -> anyhow::Result<Vec<u8>> {
    ...
    if let Some(plan) = cached {
        if !capture_plan_is_reusable(
            plan.identity,
            plan.includes_child_windows,
            identity,
            includes_child_windows,
        ) {
            let mut cache = lock_window_plan_cache();
            ...
            match capture_window_from_plan_validated(window_id, &plan) {
                ...
                Err(first_err) => {
                    ...
                    let rebuilt = match build_window_capture_plan(
                        window_id,
                        identity,
                        includes_child_windows,
                    ) {
    ...
    let plan = build_window_capture_plan(window_id, identity, includes_child_windows)?;
    {
        let mut cache = lock_window_plan_cache();
...
mod tests {
    ...
    #[test]
    fn cached_capture_plan_requires_current_sheet_composition() {
        let identity = WindowCaptureIdentity::new(42, 0, 10.0, 20.0, 370.0, 182.0);
        assert!(capture_plan_is_reusable(identity, true, identity, true));
        assert!(capture_plan_is_reusable(identity, false, identity, false));
        assert!(!capture_plan_is_reusable(identity, true, identity, false));
        assert!(!capture_plan_is_reusable(identity, false, identity, true));
        let recycled = WindowCaptureIdentity::new(43, 0, 10.0, 20.0, 370.0, 182.0);
        assert!(!capture_plan_is_reusable(identity, false, recycled, false));
    }

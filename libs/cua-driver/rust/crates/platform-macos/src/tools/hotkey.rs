        ...
        description:
            "Press a key combination — e.g. `[\"cmd\", \"c\"]` for Copy, \
             ...
             • `background` (default): post the combo to the target pid WITHOUT \
               ...
               focus that exact element first. An exact window target receives a \
               temporary app-local key-window context, without changing the real \
               foreground process or raising its window.\n\
             • `foreground`: explicitly authorized exact-window activation, then \
               restore the prior frontmost. This is not an automatic retry for an \
               unverifiable background result. With an AX target or x,y, the focused field \
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
            ...
            || async move {
                tokio::task::spawn_blocking(move || {
                    ...
                    match (fg, coordinate_focus, window_id, element_ptr) {
                        ...
                        // Background: retain authenticated PID posting, but
                        // establish the already-proven exact window's app-local
                        // key context so nil-target NSMenu actions can resolve.
                        (false, _, _, _) => {
        ...
        match result {
            Ok(Ok(())) => {
                ...
                // Posting is not an effect oracle. Unknown completion must not
                // become a foreground/replay recommendation for a non-idempotent
                // chord; inspect fresh state and focus before deciding next steps.
                ...
                ToolResult::text(format!(
                    "Posted {key_display} to pid {pid}{label}. Verify the actual state and focused field before any retry; foreground delivery requires explicit authorization.{}",

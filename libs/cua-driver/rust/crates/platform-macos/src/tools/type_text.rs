        ...
        input_schema: serde_json::json!({
            ...
            "properties": {
                ...
                "delay_ms": {
                    ...
                    "minimum": 0,
                    "maximum": 200,
                    "description": "Complete Unicode character cadence in milliseconds, default 5 and minimum effective cadence 5 (including requests from 0 to 4). Larger delays remain configurable. Remote physical-key synthesis retains its default 30 and physical-event gaps. Ignored when the AX path succeeds."
                },
...
impl Tool for TypeTextTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
            ...
            && args.get("window_id").is_none()
        {
            ...
            let text =
                cua_driver_core::text_sanitize::strip_trailing_agent_protocol_tags(&input.text)
                    .into_owned();
            let delay_ms = text_synthesis_delay_ms(
        ...
        let requested_delay_ms = args.get("delay_ms").and_then(Value::as_u64);
        ...
        let delay_ms = text_synthesis_delay_ms(
            requested_delay_ms,
            text_event_route(delivery_mode, window_id.is_some(), screen_sharing_target),
        );
        ...
        if let (Some(cx), Some(cy)) = (px, py) {
            ...
            if let BackgroundKeyboardPolicy::SemanticOnly(ref refusal) = keyboard_policy {
                ...
                return background_type_text_refusal_result(
                    pid,
                    wid,
                    refusal,
                    AxAttempt::NotAttempted,
                );
            }
            let from_zoom = args
                .get("from_zoom")
        ...
        // Preserve any semantic AX attempt when the later keyboard rung is
        // refused; a keyboard refusal is not proof that no AX write ran.
        let result = match result {
            Ok(Ok(TypeTextDelivery::Refused {
                refusal,
                ax_attempt,
            })) => {
                let wid = window_id.expect("background refusals require a window target");
                return background_type_text_refusal_result(pid, wid, &refusal, ax_attempt);
            }
...
// is synchronous; Unicode posts a down/up pair then pauses for the complete
// requested cadence (at least 5ms), with no extra down/up gap. Keep the complete
...
fn text_synthesis_delay_ms(requested: Option<u64>, route: TextEventRoute) -> u64 {
    match route {
        TextEventRoute::PidUnicode | TextEventRoute::GlobalUnicode => {
            crate::input::keyboard::unicode_character_cadence_ms(
                requested.unwrap_or(crate::input::keyboard::DEFAULT_UNICODE_CADENCE_MS),
            )
        }
        TextEventRoute::GlobalPhysical => requested.unwrap_or(30),
    }
}
...
fn background_type_text_refusal_result(
    pid: i32,
    window_id: u32,
    refusal: &BackgroundRefusal,
    ax_attempt: AxAttempt,
) -> ToolResult {
    use cua_driver_core::background_input::refusal_codes;
    let mut structured = serde_json::json!({
        "code": refusal.code,
        "effect": if ax_attempt == AxAttempt::Unverifiable {
            "unverifiable"
        } else {
            "refused"
        },
    ...
    let message = if ax_attempt == AxAttempt::NotAttempted {
        // Only these visibility/ambiguity gates have an exact-window
        // foreground alternative. No AX write or character event ran.
        let advice = match refusal.code {
            refusal_codes::SAME_PID_KEYBOARD_AMBIGUITY | refusal_codes::MINIMIZED_OR_HIDDEN => {
                Some("foreground")
            }
            _ => refusal.advice,
        };
        ...
        // Even a rejected/unchanged AX attempt is not a pre-dispatch refusal.
        // Keep its evidence and never authorize blind whole-payload replay.
        structured["retryable"] = serde_json::json!(false);
        structured["escalation"] = serde_json::json!({
            "recommended": "verify_state",
            "reason": "An atomic AX write was attempted before the keyboard route was refused. Re-read the exact target before retrying; do not replay the full text blindly.",
        });
        format!(
            "Background keyboard input refused ({}) after an atomic AX attempt ({}): {}. Re-read the exact target before retrying; do not replay the full text blindly.",
            refusal.code,
            ax_attempt.as_str(),
            refusal.reason,
        )
...
fn should_stop_after_ax_attempt(ax_attempt: AxAttempt) -> bool {
    match ax_attempt {
        AxAttempt::Unverifiable => true,
        AxAttempt::NotAttempted | AxAttempt::Rejected | AxAttempt::Unchanged => false,
    }
}
...
fn unverifiable_ax_result(pid: i32, window_id: Option<u32>) -> ToolResult {
    let mut structured = serde_json::json!({
        ...
        "escalation": {
            "recommended": "verify_state",
            "reason": "The atomic AX write could not be observed. Re-read the target before deciding whether any suffix remains; do not retry blindly.",
        },
    });
    ...
    ToolResult::error(
        "type_text cannot verify the atomic AX write; no character events were synthesized. Re-read the target before deciding whether any suffix remains; do not retry blindly.",
    )
...
fn synthesis_preflight(
    ...
    let per_character_ms = match route {
        ...
        // Share the exact pacing policy with both Unicode event senders so
        // admission cannot forget a hidden per-event sleep or divide by zero.
        TextDeliveryRoute::UnicodeSynthesis => {
            crate::input::keyboard::unicode_character_cadence_ms(delay_ms)
        }
...
/// Delivery envelope for `type_text_blocking`. A refused keyboard rung may
/// follow an AX write, so its atomic attempt travels with the policy refusal.
    ...
    match decide_background_input(target, &facts, BackgroundAction::InsertText) {
        ...
        BackgroundInputDecision::Refuse(refusal) => {
            ...
            if semantic_available {
                ...
                Err(background_type_text_refusal_result(
                    pid,
                    window_id,
                    &refusal,
                    AxAttempt::NotAttempted,
...
fn type_text_blocking(
    ...
    if is_terminal_target {
        ...
        if let BackgroundKeyboardPolicy::SemanticOnly(refusal) = keyboard_policy {
            return Ok(TypeTextDelivery::Refused {
                refusal,
                ax_attempt: AxAttempt::NotAttempted,
            });
        }
    ...
    if let Some((element, owns, idx_opt)) = ax_target {
        ...
        tracing::debug!(
            "AX write was not confirmed complete for {role} \"{title}\" (err={err}); \
    ...
    // this request is restricted to it. Preserve its effect evidence while
    // refusing the process-scoped rung, which could reach a sibling window.
    if let BackgroundKeyboardPolicy::SemanticOnly(refusal) = keyboard_policy {
        return Ok(TypeTextDelivery::Refused {
            refusal,
            ax_attempt,
    ...
    if should_stop_after_ax_attempt(ax_attempt) {
...
mod tests {
    ...
    fn unicode_default_is_five_ms_but_physical_default_remains_thirty() {
        for route in [TextEventRoute::PidUnicode, TextEventRoute::GlobalUnicode] {
            assert_eq!(text_synthesis_delay_ms(None, route), 5);
            assert_eq!(text_synthesis_delay_ms(Some(0), route), 5);
            assert_eq!(text_synthesis_delay_ms(Some(30), route), 30);
        }
        assert_eq!(
            text_synthesis_delay_ms(None, TextEventRoute::GlobalPhysical),
            30
        );
        assert_eq!(
            text_synthesis_delay_ms(Some(5), TextEventRoute::GlobalPhysical),
            5
        );
        assert_eq!(
            text_synthesis_delay_ms(Some(0), TextEventRoute::GlobalPhysical),
            0
        );
    }

    #[test]
    fn unicode_synthesis_budget_matches_complete_cadence_and_checks_overflow() {
        let fast = synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, 20_000, 0).unwrap();
        assert_eq!(fast.per_character_ms, 5);
        assert_eq!(fast.estimated_duration_ms, 102_000);
        assert_eq!(fast.max_chunk_chars, 19_600);
        let slower = synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, 4_000, 30).unwrap();
        assert_eq!(slower.per_character_ms, 30);
        assert_eq!(slower.estimated_duration_ms, 122_000);
        let overflow =
            synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, usize::MAX, u64::MAX).unwrap();
        assert_eq!(overflow.estimated_duration_ms, u64::MAX);
        assert_eq!(overflow.max_chunk_chars, 0);
        assert!(synthesis_preflight(TextDeliveryRoute::AtomicAx, usize::MAX, u64::MAX).is_none());
    }

    #[test]
    fn physical_synthesis_keeps_its_existing_event_gap_budget() {
        let refusal = synthesis_preflight(TextDeliveryRoute::PhysicalSynthesis, 4_000, 30).unwrap();
        assert_eq!(refusal.per_character_ms, 54);
        assert_eq!(refusal.estimated_duration_ms, 218_000);
        assert_eq!(refusal.max_chunk_chars, 1_814);
    }
    ...
    #[test]
    fn background_text_refusal_marks_zero_ax_attempt_and_exact_target() {
        use cua_driver_core::background_input::refusal_codes;
        for code in [
            ...
            let refusal = BackgroundRefusal {
                code,
                reason: "exact-window keyboard route was refused".into(),
                advice: Some("accessibility"),
            };
            let result =
                background_type_text_refusal_result(42, 7, &refusal, AxAttempt::NotAttempted);
    ...
    #[test]
    fn background_text_refusal_preserves_unverifiable_ax_effect() {
        let refusal = BackgroundRefusal {
            code: cua_driver_core::background_input::refusal_codes::SAME_PID_KEYBOARD_AMBIGUITY,
            reason: "another eligible window exists".into(),
            advice: Some("accessibility"),
        };
        let result = background_type_text_refusal_result(42, 7, &refusal, AxAttempt::Unverifiable);
        assert_eq!(result.is_error, Some(true));
        let structured = result
            .structured_content
            .expect("structured unknown effect");
        assert_eq!(structured["effect"], "unverifiable");
        assert_eq!(structured["atomic_ax_effect"], "unverifiable");
        assert_eq!(structured["retryable"], false);
        assert_eq!(structured["escalation"]["recommended"], "verify_state");
        assert_eq!(structured["pid"], 42);
        assert_eq!(structured["window_id"], 7);
        assert!(structured.get("delivered_chars").is_none());
        assert!(structured.get("retry_from_character").is_none());
    }

    #[test]
    fn background_text_refusal_never_authorizes_replay_after_ax_attempt() {
        let refusal = BackgroundRefusal {
            code: cua_driver_core::background_input::refusal_codes::MINIMIZED_OR_HIDDEN,
            reason: "target visibility is not proven".into(),
            advice: Some("accessibility"),
        };
        for attempt in [
            AxAttempt::Rejected,
            ...
            let result = background_type_text_refusal_result(42, 7, &refusal, attempt);
            let structured = result.structured_content.expect("structured AX attempt");
            assert_eq!(structured["atomic_ax_effect"], attempt.as_str());
            assert_eq!(structured["retryable"], false);
            assert_eq!(structured["escalation"]["recommended"], "verify_state");
        }
    }
    ...
    #[test]
    fn background_text_refusal_does_not_promote_other_policy_codes() {
        let refusal = BackgroundRefusal {
            code: cua_driver_core::background_input::refusal_codes::ELEMENT_OUTSIDE_TARGET_WINDOW,
            reason: "element ancestry is not proven".into(),
            advice: Some("get_window_state"),
        };
        let result = background_type_text_refusal_result(42, 7, &refusal, AxAttempt::NotAttempted);
        let structured = result.structured_content.expect("structured refusal");
        assert_eq!(structured["code"], refusal.code);
        assert_eq!(structured["atomic_ax_effect"], "not_attempted");
        assert_eq!(structured["escalation"]["recommended"], "get_window_state");
    }

    #[test]
    fn background_text_unverifiable_ax_stops_before_character_synthesis() {
        assert!(should_stop_after_ax_attempt(AxAttempt::Unverifiable));
    }

    #[test]
    fn background_text_known_ax_attempts_preserve_synthesis_eligibility() {
        for attempt in [
            AxAttempt::NotAttempted,
            AxAttempt::Rejected,
            AxAttempt::Unchanged,
        ] {
            assert!(!should_stop_after_ax_attempt(attempt));
        }
    }
    ...
    fn background_text_unknown_ax_result_has_no_replay_claim() {
        for window_id in [None, Some(7)] {
            ...
            assert_eq!(structured["effect"], "unverifiable");
            assert_eq!(structured["atomic_ax_effect"], "unverifiable");
            assert_eq!(structured["synthesized_chars"], 0);
            assert_eq!(structured["retryable"], false);
            assert_eq!(structured["escalation"]["recommended"], "verify_state");
            assert_eq!(structured["pid"], 42);
            match window_id {
                Some(id) => assert_eq!(structured["window_id"], id),
                None => assert!(structured.get("window_id").is_none()),
            }
            assert!(structured.get("delivered_chars").is_none());
            assert!(structured.get("retry_from_character").is_none());
        }
    ...
    fn oversized_synthesis_is_refused_before_the_terminal_event_path() {
        ...
        assert_eq!(ax_attempt, AxAttempt::NotAttempted);
        assert_eq!(refusal.requested_chars, 20_000);
        assert_eq!(refusal.estimated_duration_ms, 102_000);
        assert_eq!(refusal.max_chunk_chars, 19_600);
    }

    #[test]
    fn synthesis_preflight_accepts_the_exact_transport_safe_boundary() {
        assert!(synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, 19_600, 0).is_none());
        assert!(synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, 19_601, 0).is_some());
    }
    ...
    fn refusal_diagnostics_distinguish_safe_chunking_from_indeterminate_ax() {
        let refusal = synthesis_preflight(TextDeliveryRoute::UnicodeSynthesis, 20_000, 0)

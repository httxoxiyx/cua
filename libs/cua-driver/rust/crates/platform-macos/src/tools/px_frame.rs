...
pub enum PxFrameError {
    /// The most recent image is a separate application-menu crop, not a
    /// document coordinate frame. Fresh AX menu commands remain available.
    ApplicationMenuCoordinatesUnsupported { window_id: u32 },
...
pub fn resolve_window_px_frame(window_id: u32) -> Result<WindowPxFrame, PxFrameError> {
    if crate::ax::application_menu::menu_image_coordinates_withheld(window_id) {
        return Err(PxFrameError::ApplicationMenuCoordinatesUnsupported { window_id });
    }
...
pub fn refusal(error: &PxFrameError) -> ToolResult {
    match error {
        PxFrameError::ApplicationMenuCoordinatesUnsupported { .. } => ToolResult::error(
            "The current screenshot shows an application menu, not the document pixel frame. Use a freshly observed AX menu action, or close the menu and obtain a new document screenshot before pointer input.",
...
pub fn error_structured(error: &PxFrameError) -> serde_json::Value {
    match error {
        PxFrameError::ApplicationMenuCoordinatesUnsupported { window_id } => serde_json::json!({
            "code":"application_menu_coordinates_unsupported",
            "effect":"refused","retryable":true,"window_id":window_id,
            "suggestion":"Use a freshly observed AX menu action, or close the menu and re-observe the document image before pointer input."
        }),
...
mod tests {
    ...
    fn application_menu_pixels_are_refused_before_any_capture_or_transform() {
        let image = crate::ax::application_menu::ApplicationMenuImage {
            ...
            document_bounds: WindowBounds {
                x: 504.0,
                y: 63.0,
                width: 700.0,
                height: 892.0,
            },
            menu_bounds: WindowBounds {
                x: 215.0,
                y: 34.0,
                width: 203.0,
                height: 117.0,
        ...
        assert!(crate::ax::application_menu::remember_observed_image(
            42,
            image.document_window_id,
            Some(&image)
        ));
        for window_id in [image.document_window_id, image.menu_window_id] {
            let error = resolve_window_px_frame(window_id).unwrap_err();
            assert_eq!(
                error,
                PxFrameError::ApplicationMenuCoordinatesUnsupported { window_id }
            );
            let result = refusal(&error);
            assert_eq!(
                result.structured_content.as_ref().unwrap()["code"],
                "application_menu_coordinates_unsupported"
            );
            assert_eq!(
                result.structured_content.as_ref().unwrap()["effect"],
                "refused"
            );
        }
        crate::ax::application_menu::remember_observed_image(42, image.document_window_id, None);
    }

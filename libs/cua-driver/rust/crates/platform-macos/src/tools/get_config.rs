...
impl Tool for GetConfigTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        ToolResult::text("cua-driver-rs configuration").with_structured(serde_json::json!({
            ...
            // Runtime evidence, unlike the persisted preference above. This
            // never captures a frame or changes the current preview target.
            "pip_runtime": crate::pip::diagnostic_state(),
            "foreground_activity": crate::foreground_activity::diagnostic_state(),
        }))
    }
}

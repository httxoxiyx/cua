...
unsafe fn ax_window_records(
    ...
    {
        if let Some(sheet) =
            super::attached_sheet::copy_focused_attached_sheet(pid, target_window_id)
        {
    ...
    let (records, app_hidden, element) = unsafe {
        ...
        if app.is_null() {
            ...
            let element = element_ptr.map(|ptr| match element_window_id(ptr as AXUIElementRef) {
                ...
                Some(_)
                    if super::attached_popover::proves_attached_popover(
                        pid,
                        window_id,
                ...
                None if super::application_menu::proves_application_menu(
                    pid,
                    window_id,
                    ptr as AXUIElementRef,
                ) =>

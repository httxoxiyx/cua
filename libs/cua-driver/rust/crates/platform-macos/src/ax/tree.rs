...
fn walk_tree_bounded_with_projection(
    ...
    unsafe {
        ...
        // AppKit can expose the focused Save sheet only under its host's
        // AXChildren. Include that exact sheet after proving the reciprocal
        // attachment, rather than substituting its host's tree for this id.
        if let Some(sheet) =
            window_id.and_then(|wid| super::attached_sheet::copy_focused_attached_sheet(pid, wid))
        {
            if top_level
                .iter()
                .any(|&element| CFEqual(element as CFTypeRef, sheet as CFTypeRef) != 0)
            {

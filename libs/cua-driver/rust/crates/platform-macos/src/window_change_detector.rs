...
/// Input acknowledgement is not the lifetime of focus-steal protection.
/// The next input still revalidates its exact target; do not make every stable
/// form field wait a full second just in case it opens another window.
const INPUT_REPORT_TIMEOUT: Duration = Duration::from_millis(100);

/// Default inter-poll interval. Matches Swift's 50ms.
const DEFAULT_POLL_INTERVAL: Duration = Duration::from_millis(50);

fn input_report_timeout(scope: SuppressionScope) -> Duration {
    match scope {
        SuppressionScope::None | SuppressionScope::Target(_) => INPUT_REPORT_TIMEOUT,
        // Launch/setup owns a broader scope and keeps its historical timing.
        SuppressionScope::Wildcard | SuppressionScope::WildcardAllowing(_) => DEFAULT_TIMEOUT,
    }
}
...
impl Snapshot {
    ...
    /// Fast acknowledgement for ordinary input, without shortening the
    /// target-only focus protection. The deferred lease is explicitly
    /// cancellable by a later authorized foreground action for that target.
    /// Wildcard launch/setup and the legacy detect APIs are unchanged.
    pub async fn detect_input_async(self) -> Changes {
        tokio::task::spawn_blocking(move || {
            let timeout = input_report_timeout(self.suppression_scope);
...
mod tests {
    ...
    fn ordinary_input_reports_within_one_hundred_milliseconds() {
        assert_eq!(
            input_report_timeout(SuppressionScope::None),
            Duration::from_millis(100)
        );
        assert_eq!(
            input_report_timeout(SuppressionScope::Target(42)),
            Duration::from_millis(100)
        );
    ...
    #[test]
    fn wildcard_setup_keeps_its_full_observation_window() {
        assert_eq!(
            input_report_timeout(SuppressionScope::Wildcard),
            DEFAULT_TIMEOUT
        );
        assert_eq!(

...
/// Space queries. Use this for liveness checks that must retain minimized and
/// off-Space targets. Do not infer visual foreground order by filtering this
/// snapshot: the all-window response can differ from the on-screen ordering.
/// Use [`visible_windows_including_accessory_layers_with_snapshot`] for that.
...
/// Raw on-screen WindowServer snapshot for visual front-to-back ordering,
/// including accessory layers, with success status and no per-window Space
/// queries. This snapshot must not be used to retire off-screen targets.
pub(crate) fn visible_windows_including_accessory_layers_with_snapshot() -> WindowEnumeration {
    enumerate_windows(
        kCGWindowListOptionOnScreenOnly | kCGWindowListExcludeDesktopElements,

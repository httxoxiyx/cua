...
pub(crate) fn unicode_character_cadence_ms(requested_ms: u64) -> u64 {
    requested_ms.max(DEFAULT_UNICODE_CADENCE_MS)
}

fn dispatch_unicode_text_with<Event, Error>(
    text: &str,
...
/// Type literal Unicode text to `pid` with a five-millisecond character cadence.
pub fn type_text(pid: i32, text: &str) -> anyhow::Result<()> {
    type_text_with_delay(pid, text, DEFAULT_UNICODE_CADENCE_MS)
}

/// Type Unicode characters with one `max(inter_char_delay_ms, 5)` pause after
/// each down/up pair. There is no additional hidden down/up timing floor.
...
fn unicode_keyboard_event(
    source: &CGEventSource,
    value: &str,
    key_down: bool,
) -> anyhow::Result<CGEvent> {
    let event = CGEvent::new_keyboard_event(source.clone(), 0, key_down)
        .map_err(|_| anyhow::anyhow!("CGEvent Unicode keyboard event failed"))?;
    if key_down {
        crate::foreground_activity::check_if_foreground_episode()?;
    }
    crate::foreground_activity::mark_generated(&event);
    event.set_string(value);
    // Do not infer Shift from uppercase text or inherit the user's modifiers.
    // The literal Unicode payload is identical on both sides of the pair.
...
/// Run one already-authorized key operation with the exact background window's
/// app-local responder context. Callers must retain the background target gate
/// and mutation lease across this entire call. No foreground or retry route is
/// available here; the underlying authenticated event transport is unchanged.
pub(crate) fn with_background_window_context<R>(
    pid: i32,
    window_id: Option<u32>,
    action: impl FnOnce() -> anyhow::Result<R>,
) -> anyhow::Result<R> {
    let Some(window_id) = window_id else {
        return action();
    };
    let target_is_frontmost = crate::input::skylight::front_process_matches(pid, window_id)
        .ok_or_else(|| anyhow::anyhow!("cannot verify foreground before background key routing"))?;
    let dispatched_at = std::cell::Cell::new(std::time::Instant::now());
    super::background_keyboard::dispatch_with_target_context(
        target_is_frontmost,
        || crate::input::skylight::begin_synthetic_target_focus(pid, window_id),
        |context| context.make_window_key(),
        || {
            dispatched_at.set(std::time::Instant::now());
            action()
        },
        || {
            // Reuse any existing AX-oracle wait inside the action. Otherwise
            // allow one existing 40-ms routing interval before target cleanup;
            // do not add the mouse path's longer renderer delay to every key.
            let remaining =
                std::time::Duration::from_millis(40).saturating_sub(dispatched_at.get().elapsed());
...
///
/// This legacy alternative is not a universal NSMenu requirement: authenticated
/// events reach AppKit menus when their target responder context is established.
/// Background tools retain the authenticated route rather than replaying an
/// uncertain chord through this alternative transport.
...
pub(super) fn with_global_modifier_keys<T>(
    ...
    for modifier in modifiers {
        ...
        let next_flags = active_flags | flag;
        if let Err(error) =
            post_global_key(&source, key_code, true, next_flags, CGEventTapLocation::HID)
        {
...
pub(super) fn with_pid_modifier_keys<T>(
    ...
    for modifier in modifiers {
        ...
        let next_flags = active_flags | flag;
        if let Err(error) = post_key(pid, key_code, true, next_flags) {
...
pub fn press_key_bare_global(key: &str, modifiers: &[&str]) -> anyhow::Result<()> {
    ...
    let events = bare_chord_transitions(key_code, &modifier_codes)
        ...
        .collect::<anyhow::Result<Vec<_>>>()?;
    post_bare_sequence(&events, &bare_chord_transitions(key_code, &modifier_codes))?;
    Ok(())
}

fn post_bare_sequence(events: &[CGEvent], transitions: &[(u16, bool)]) -> anyhow::Result<()> {
    let mut held = Vec::new();
    let result = (|| {
        for (event, &(code, down)) in events.iter().zip(transitions) {
            crate::foreground_activity::post_global(event, !down)?;
            if down {
                held.push(code);
            } else {
    ...
    // Reuse preallocated release events, including on interruption. Do not
    // allocate a key-up only after a key-down has already reached the OS.
    for code in held.into_iter().rev() {
        if let Some((up, _)) = events
            .iter()
            .zip(transitions)
            .find(|(_, &(key, down))| key == code && !down)
        {
            let _ = crate::foreground_activity::post_global(up, true);
        }
    }
    result
}
...
mod tests {
    ...
    #[test]
    fn unicode_typing_preserves_explicit_delay_with_five_ms_minimum() {
        for (requested, expected) in [(0, 5), (1, 5), (5, 5), (30, 30), (200, 200)] {
            ...
            .unwrap();
            assert_eq!(*pauses.borrow(), [expected], "requested delay {requested}");
        }
    }

    #[test]
    fn unicode_takeover_while_allocating_up_prevents_down_dispatch() {
        let revoked = Cell::new(false);
        let posted = RefCell::new(Vec::new());
        let result = dispatch_unicode_text_with(
            "ab",
            5,
            |value, down| {
                if !down {
        ...
        assert_eq!(result, Err("activity interrupted"));
        assert!(posted.borrow().is_empty());
    }

    #[test]
    fn unicode_typing_constructs_the_pair_before_posting_either_event() {
        let posted = RefCell::new(Vec::new());
        let pauses = RefCell::new(Vec::new());
        ...
        .unwrap_err();
        assert_eq!(error, "key-up creation failed");
        assert_eq!(
            *posted.borrow(),

...
pub(crate) fn restore_exact_window(pid: i32, window: u32) -> bool {
    if !submit_exact_window_restore(pid, window) {
        return false;
    }
    let mut psn = [0_u8; 8];
    get_process_psn_for_window(window, pid, &mut psn) && await_exact_window_ready(pid, window, psn)
}
...
fn with_foreground_hid_activation_inner(
    ...
    if crate::apps::frontmost_pid() != Some(target_pid)
        || crate::ax::bindings::focused_window_id_of_pid(target_pid) != Some(target_wid)
    {
        episode.check()?;
        let _pip_activation = crate::pip::begin_temporary_activation(target_pid, target_wid, None);
        ...
        {
            anyhow::bail!(
                "exact foreground target did not become ready; no automatic restore after failure"
            );
        }
        crate::foreground_activity::check_input()?;
        let result = action();
        std::thread::sleep(std::time::Duration::from_millis(40));
        return episode.finish(result);
    }
    crate::foreground_activity::check_input()?;
    episode.finish(action())
}

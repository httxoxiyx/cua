...
impl NativeInputState {
    ...
    fn prepare_target(&self) -> Result<()> {
        if self.config.delivery_mode == InteractiveDeliveryMode::PersistentForeground {
            return Err(InteractiveInputError::Native("foreground_activity_unavailable: persistent interactive foreground requires an explicit bounded episode lifecycle".into()));
        }
        Ok(())
    }

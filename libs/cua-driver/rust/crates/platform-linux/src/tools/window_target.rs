...
/// Preserve the process-instance-bound Wayland discovery fallback during the
/// initial preflight, while never accepting a dead requested process or
/// hiding a currently observed foreign owner.
pub(crate) fn preflight_owner_pid(
    requested_pid: u32,
    process_is_live: bool,
    current_owner_pid: Option<u32>,
    remembered_same_process_window: bool,
) -> Option<u32> {
    match current_owner_pid {
        Some(owner_pid) if owner_pid != requested_pid => Some(owner_pid),
        Some(owner_pid) if process_is_live => Some(owner_pid),
        Some(_) => None,
        None if process_is_live && remembered_same_process_window => Some(requested_pid),
        None => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn preflight_keeps_wayland_fallback_but_rejects_dead_or_foreign_owners() {
        assert_eq!(preflight_owner_pid(42, true, None, true), Some(42));
        assert_eq!(preflight_owner_pid(42, false, Some(42), true), None);
        assert_eq!(preflight_owner_pid(42, true, Some(99), true), Some(99));
        assert_eq!(preflight_owner_pid(42, true, None, false), None);
    }
}

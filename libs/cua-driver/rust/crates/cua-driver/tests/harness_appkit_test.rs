...
//!   - ns_menubar     : main menubar item enumerable and background semantic action
//!
...
impl Harness {
    ...
    fn launch_with_keyboard_oracle(oracle_path: &Path, menu_route: &str) -> Self {
        let exe = harness_exe();
        assert!(
            exe.exists(),
            "required AppKit harness is missing at {exe:?}"
        );
        let app = Command::new(&exe)
            .env("CUA_APPKIT_KEYBOARD_ORACLE", oracle_path)
            .env("CUA_APPKIT_KEYBOARD_ROUTE", menu_route)
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .unwrap_or_else(|error| panic!("launch AppKit keyboard harness {exe:?}: {error}"));
        let harness = Self {
            pid: app.id(),
            _app: app,
        };
        let ready = wait_for_keyboard_oracle(oracle_path, "ready");
        assert_eq!(ready["pid"].as_u64(), Some(harness.pid as u64));
        assert_eq!(ready["menu_route"], menu_route);
        assert_eq!(ready["select_all_count"].as_u64(), Some(0));
        assert_eq!(ready["selection_length"].as_u64(), Some(0));
        // Launch/focus is setup. run_with_background_oracles establishes and
        // validates the real background boundary before the measured hotkey.
        harness
    }
}

fn wait_for_keyboard_oracle(path: &Path, event: &str) -> serde_json::Value {
    wait_for_keyboard_oracle_matching(path, event, |_| true)
}

fn wait_for_keyboard_oracle_matching(
    path: &Path,
    event: &str,
    ...
    loop {
        ...
        if let Some(record) = contents
            .lines()
            .filter_map(|line| serde_json::from_str::<serde_json::Value>(line).ok())
            .find(|record| record["event"] == event && matches(record))
        {
            return record;
        }
        assert!(
            std::time::Instant::now() < deadline,
            "missing AppKit keyboard event {event:?}; actual oracle: {contents}"
        );
...
#[test]
#[ignore]
fn harness_appkit_application_menu_background() {
    run_background_case(
        "application_menu",
        DriverRoute::MacosAxAction,
        |pid, wid, driver| {
            let first = snapshot_elements(driver, pid, wid);
            let stale_token = element_token_by_id(&first, "menu-test-item");
            let fresh = snapshot_elements(driver, pid, wid);
            let stale = driver.call(
                "click",
                serde_json::json!({
                    "pid": pid, "element_token": stale_token, "delivery_mode": "background"
                }),
            );
            assert!(stale.is_error(), "stale menu token was accepted");
            assert_eq!(
                stale.structured()["refusal"]["code"].as_str(),
                Some("stale_element_token")
            );

            // Menu membership must not relax generic AX value mutation.
            let refused = driver.call(
                "set_value",
                serde_json::json!({
                    "pid": pid, "element_token": element_token_by_id(&fresh, "menu-test-item"),
                    "value": "must not write"
                }),
            );
            assert!(refused.is_error(), "menu proof authorized set_value");
            assert_eq!(
            ...
            assert!(before.tree_text().contains("menu_action=none"));
            let response = driver.call(
                "click",
                serde_json::json!({
                    "pid": pid, "element_token": element_token_by_id(&before, "menu-test-item"),
                    "action": "press", "delivery_mode": "background"
                }),
            );
            assert!(
                !response.is_error(),
                "background menu action failed: {}",
                response.text()
            );
            ...
            loop {
                ...
                if after.tree_text().contains("menu_action=application_menu:1") {
                    break;
                }
                assert!(
                    std::time::Instant::now() < deadline,
                    "menu callback did not run exactly once: {}",
                    after.tree_text()
                );
...
/// A transport-success result is insufficient: a standard nil-target Select
/// All menu action must select the actual field editor through responder routing.
#[test]
#[ignore]
fn harness_appkit_background_command_a_selects_text() {
    run_appkit_background_command_a("responder_chain");
}

/// This positive control bypasses responder routing deliberately. It cannot
/// replace the nil-target test above or establish that Pages handles the chord.
#[test]
#[ignore]
fn harness_appkit_background_command_a_explicit_target_positive_control() {
    run_appkit_background_command_a("explicit_target");
}

fn run_appkit_background_command_a(menu_route: &str) {
    let case = native_background_case(
        "appkit",
        &format!("hotkey_select_all_{menu_route}"),
    ...
    execute_case(case, |evidence| {
        let mut driver = McpDriver::spawn_macos_daemon_proxy_named(&cell_id)
            .expect("start installed macOS daemon proxy");
        *evidence = recording_evidence(driver.recording_dir());
        let oracle_dir = tempfile::tempdir().expect("create keyboard oracle directory");
        let oracle_path = oracle_dir.path().join("keyboard-events.jsonl");
        let harness = Harness::launch_with_keyboard_oracle(&oracle_path, menu_route);
        let (wid, _) = driver
            .find_window(harness.pid as i64, "CuaTestHarness AppKit")
            .expect("AppKit main window not found");
        ...
        let (_, passed) = run_with_background_oracles(
            ...
            |driver| {
                let before =
                    wait_for_keyboard_oracle_matching(&oracle_path, "heartbeat", |record| {
                        record["active"] == false
                            && record["application_key_window_id"].as_i64() == Some(-1)
                            && record["application_main_window_id"].as_i64() == Some(-1)
                    });
                ...
                assert!(
                    !result.is_error(),
                    "background Cmd+A refused: {}",
                    result.raw
                );
                assert_eq!(result.structured()["effect"], "unverifiable");
                let key_down =
                    wait_for_keyboard_oracle_matching(&oracle_path, "key_down", |record| {
                        record["preparation_complete"] == true
                            && record["command"] == true
                            && record["key_code"].as_u64() == Some(0)
                    });
                assert_eq!(key_down["pid"].as_u64(), Some(harness.pid as u64));
                // Target-local AppKit activation is not WindowServer foreground.
                assert_eq!(key_down["frontmost_pid"], before["frontmost_pid"]);
                let selected = wait_for_keyboard_oracle_matching(
                    &oracle_path,
                    "selection_changed",
                    |record| {
                        record["preparation_complete"] == true
                            && record["selection_location"].as_u64() == Some(0)
                            && record["selection_length"] == record["text_utf16_length"]
                            && record["selection_length"].as_u64().unwrap_or_default() > 0
                    },
                );
                println!("AppKit Cmd+A field-editor selection evidence: {selected}");
                assert_eq!(selected["pid"].as_u64(), Some(harness.pid as u64));
                assert_eq!(selected["window_id"].as_u64(), Some(wid));
                assert_eq!(selected["menu_route"], menu_route);
                assert_eq!(
                    selected["source"],
                    "NSTextView.didChangeSelectionNotification"
                );
                assert_eq!(
                    selected["select_all_count"].as_u64(),
                    Some(u64::from(menu_route == "explicit_target"))
                );
                assert_eq!(selected["editor_is_first_responder"], true);
                assert_eq!(selected["selection_location"].as_u64(), Some(0));
                assert_eq!(selected["selection_length"], selected["text_utf16_length"]);
                assert!(selected["selection_length"].as_u64().unwrap_or_default() > 0);
                assert_eq!(selected["text"], "Cua background selection fixture");
                assert_eq!(selected["frontmost_pid"], before["frontmost_pid"]);
                assert!(
                    key_down["monotonic_seconds"]
                        .as_f64()
                        .expect("key event timestamp")
                        <= selected["monotonic_seconds"]
                            .as_f64()
                            .expect("selection timestamp")
                );
                let cleaned =
                    wait_for_keyboard_oracle_matching(&oracle_path, "heartbeat", |record| {
                        record["monotonic_seconds"].as_f64().unwrap_or_default()
                            > selected["monotonic_seconds"].as_f64().unwrap_or(f64::MAX)
                            && record["active"] == false
                            && record["application_key_window_id"].as_i64() == Some(-1)
                            && record["application_main_window_id"].as_i64() == Some(-1)
                    });
                assert_eq!(cleaned["frontmost_pid"], before["frontmost_pid"]);
                assert_eq!(cleaned["selection_location"].as_u64(), Some(0));
                assert_eq!(cleaned["selection_length"], cleaned["text_utf16_length"]);
                println!("AppKit Cmd+A target cleanup evidence: {cleaned}");
            },
        )
        .unwrap_or_else(|error| panic!("background desktop contract failed: {error}"));
        Observation::delivered_with_fixture_state(passed)

...
### Native-only builds and language bindings

The targeted `cua-driver` and `cursor-theme-cli` builds do not enable UniFFI.
Their Rust SDK, C ABI, Serde models, schemas, and runtime behavior remain
available. This is a build boundary, not a change to tool or permission policy.

`cua-driver-sdk` and `cua-driver-contract` retain their `ffi` feature by default
for existing language-binding consumers. Rust-only consumers can set
`default-features = false`; `cua-driver-sdk/ffi` also enables the contract's
converters. The existing binding-generation command continues to build the SDK
with its defaults.

Cargo features are additive: building the default SDK, contract, or bindgen
together with the native targets (for example, `--workspace`) can include
UniFFI again. Qualify the exact selected build graph, not the presence of UniFFI
in `Cargo.lock`.
From the repository root, check native and SDK dependency boundaries with:

```bash
python3 -m unittest discover -s libs/cua-driver/scripts/tests -p test_ffi_features.py
```

This offline check includes build and test dependencies but does not replace
compilation, binding generation, or native behavior tests.

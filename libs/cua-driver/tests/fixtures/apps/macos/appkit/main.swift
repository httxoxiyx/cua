...
final class HarnessWindowController: NSObject, NSTextFieldDelegate, NSTableViewDataSource, NSTableViewDelegate, NSMenuItemValidation {
    ...
    private var applicationMenuCount = 0
    ...
    @objc func onApplicationMenu(_ sender: NSMenuItem) {
        applicationMenuCount += 1
        menuActionLabel.stringValue = "menu_action=application_menu:\(applicationMenuCount)"
    }
...
func installMenuBar(target: HarnessWindowController) {
    ...
    let testItem = NSMenuItem(title: kMenuItemTitle,
                             action: #selector(HarnessWindowController.onApplicationMenu(_:)),
                             keyEquivalent: "")
    ...
    installKeyboardSelectionProbe(target: target, mainMenu: main)
